# **************************************************************************** #
#                                                                              #
#                                                         :::      ::::::::    #
#    makefile                                           :+:      :+:    :+:    #
#                                                     +:+ +:+         +:+      #
#    By: gchuan <marvin@42.fr>                      +#+  +:+       +#+         #
#                                                 +#+#+#+#+#+   +#+            #
#    Created: 2023/07/06 21:46:14 by gchuan            #+#    #+#              #
#    Updated: 2023/07/06 21:52:07 by gchuan           ###   ########.fr        #
#                                                                              #
# **************************************************************************** #

CC = cc
CFLAGS = -Wall -Wextra -Werror

NAME 		= libft.a
SRCS 		= ft_atoi.c ft_isascii.c ft_memcpy.c ft_putstr_fd.c ft_strlcat.c \
			ft_strrchr.c ft_bzero.c ft_isdigit.c ft_memmove.c ft_split.c \
			ft_strlcpy.c ft_strtrim.c ft_calloc.c ft_isprint.c ft_memset.c \
			ft_strchr.c ft_strlen.c ft_substr.c ft_itoa.c ft_putchar_fd.c \
			ft_strdup.c ft_strmapi.c ft_tolower.c ft_isalnum.c ft_memchr.c \
			ft_putendl_fd.c ft_striteri.c ft_strncmp.c ft_toupper.c \
			ft_isalpha.c ft_memcmp.c ft_putnbr_fd.c ft_strjoin.c ft_strnstr.c
BONUSSRCS 	= ft_lstnew.c ft_lstadd_front.c ft_lstsize.c ft_lstlast.c \
			ft_lstadd_back.c ft_lstdelone.c ft_lstclear.c ft_lstiter.c \
			ft_lstmap.c

LIBC 		= ar rcs
OBJS 		= ${SRCS:.c=.o}
BONUSOBJS 	= ${BONUSSRCS:.c=.o}
RM   		= rm -f

all: $(NAME)

$(OBJS): $(SRCS)
	$(CC) $(CFLAGS) -c $(SRCS)

${NAME}: $(OBJS)
	$(LIBC) $(NAME) $(OBJS)
	
$(BONUSOBJS): $(BONUSSRCS)
	$(CC) $(CFLAGS) -c $(BONUSSRCS)

bonus: $(BONUSOBJS)
	$(LIBC) $(NAME) $(BONUSOBJS)

clean:
	$(RM) $(OBJS) $(NAME)

fclean: clean
	$(RM) $(NAME)

re:	fclean all

.phony: all, clean, fclean, re, bonus
